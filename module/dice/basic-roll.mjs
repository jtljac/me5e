...
export default class BasicRoll extends Roll {
  ...
  static async buildConfigure(config={}, dialog={}, message={}) {
     ...
     * method (e.g. `me5e.preRollSkill`, `me5e.preRollAbilityCheck`, `me5e.preRoll`). Exact contents of the
     * configuration object will also change based on the roll type, but the same objects will always be present.
     * @function me5e.preRoll
     * @memberof hookEvents
    ...
    for ( const hookName of config.hookNames ) {
      if ( Hooks.call(`me5e.preRoll${hookName.capitalize()}`, config, dialog, message) === false ) return [];
      if ( Hooks.call(`me5e.preRoll${hookName.capitalize()}V2`, config, dialog, message) === false ) return [];
    }
    ...
    if ( dialog.configure === false ) {
      rolls = config.rolls?.map((r, index) => {
        ...
        for ( const hookName of config.hookNames ) {
          Hooks.callAll(`me5e.postBuild${hookName.capitalize()}RollConfig`, config, r, index);
        }
    ...
    const rollType = foundry.utils.getProperty(message, "data.flags.me5e.roll.type");
     ...
     * Multiple hooks may be called depending on the rolling method (e.g. `me5e.postSkillCheckRollConfiguration`,
     * `me5e.postAbilityTestRollConfiguration`, and `me5e.postRollConfiguration` for skill checks). Exact contents of
     * the configuration object will also change based on the roll type, but the same objects will always be present.
     * @function me5e.postRollConfiguration
     * @memberof hookEvents
     ...
     */
    for ( const hookName of config.hookNames ) {
      const name = `me5e.post${hookName.capitalize()}RollConfiguration`;
  ...
  static async buildPost(rolls, config, message) {
    ...
    if ( messageId ) foundry.utils.setProperty(message.data, "flags.me5e.originatingMessage", messageId);
