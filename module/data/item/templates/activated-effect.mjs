 ...
 * @property {string} activation.type       Activation type as defined in `ME5E.abilityActivationTypes`.
 ...
 * @property {string} duration.units        Time duration period as defined in `ME5E.timePeriods`.
 ...
 * @property {string} target.units          Units used for value and width as defined in `ME5E.distanceUnits`.
 * @property {string} target.type           Targeting mode as defined in `ME5E.targetTypes`.
 ...
 * @property {string} range.units           Units used for value and long as defined in `ME5E.distanceUnits`.
 ...
 * @property {string} uses.per              Recharge time for limited uses as defined in `ME5E.limitedUsePeriods`.
 * @property {object} consume               Effect's resource consumption.
 * @property {string} consume.type          Type of resource to consume as defined in `ME5E.abilityConsumptionTypes`.
...
export default class ActivatedEffectTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      ...
      target: new foundry.data.fields.SchemaField({
        ...
        prompt: new foundry.data.fields.BooleanField({initial: true, label: "ME5E.TemplatePrompt"})
      }, {label: "ME5E.Target"}),
  ...
  static ItemUsesField = class ItemUsesField extends foundry.data.fields.SchemaField {
    constructor(extraSchema, options) {
      super(SystemDataModel.mergeSchema({
        ...
        prompt: new foundry.data.fields.BooleanField({initial: true, label: "ME5E.LimitedUsesPrompt"})
      }, extraSchema), options);
    }
  };
   ...
   */
  get hasAreaTarget() {
    return this.isActive && (this.target.type in CONFIG.ME5E.areaTargetTypes);
  }
  ...
  get hasIndividualTarget() {
    return this.isActive && (this.target.type in CONFIG.ME5E.individualTargetTypes);
  }
   ...
   */
  get hasLimitedUses() {
    return this.isActive && (this.uses.per in CONFIG.ME5E.limitedUsePeriods) && (this.uses.max > 0);
  }
