...
export default class Actor5e extends Actor {
  ...
  _prepareBaseModifiers(actorData) {
    ...
    for (const modVar of Actor5e.modifiers) {
      ...
      modObject.modifiers.mods = new ModList();
      // User Mods
      for (const mod of modObject.modifiers?.user) {
        modObject.modifiers.mods.push(new Modifier(mod.name, mod.type, mod.formula))
      }
    }
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    const slots = ME5E.SPELL_SLOT_TABLE[Math.min(levels, ME5E.SPELL_SLOT_TABLE.length) - 1] || [];
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("me5e", "remarkableAthlete") && ME5E.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  rollSkill(skillId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.format("ME5E.SkillPromptTitle", {skill: CONFIG.ME5E.skills[skillId]})}: ${this.name}`,
      chooseModifier: true,
   ...
   * @param {string} ability  Ability key as defined in `CONFIG.ME5E.abilities`.
