...
export default class ProficiencySelector extends TraitSelector {
    ...
    async getData() {
        ...
        this.options.choices = CONFIG.ME5E[`${this.options.type}Proficiencies`];
        const data = super.getData();

        const pack = game.packs.get(CONFIG.ME5E.sourcePacks.ITEMS);
        const ids = CONFIG.ME5E[`${this.options.type}Ids`];
        const map = CONFIG.ME5E[`${this.options.type}ProficienciesMap`];
        if(ids !== undefined) {
        ...
        if(this.options.type === "tool") {
            data.choices["vehicle"].children = Object.entries(CONFIG.ME5E.vehicleTypes).reduce((obj, [key, label]) => {
                obj[key] = {
                    label: label,
                    chosen: attr ? value.includes(key) : false
                };
