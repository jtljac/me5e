...
export default class Actor5e extends SystemDocumentMixin(Actor) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/me5e/icons/svg/documents/actor.svg";
   ...
   */
  get coverBonus() {
    const { coverHalf, coverThreeQuarters } = CONFIG.ME5E.statusEffects;
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("me5e", "item");
  ...
  get summonedCreatures() {
    return me5e.registry.summons.creatures(this);
  }
  ...
  _initializeSource(source, options={}) {
    ...
    if ( (source.type === "group") && (source.system?.type?.value === "encounter") ) {
      ...
      foundry.utils.setProperty(source, "flags.me5e.persistSourceMigration", true);
    }
    ...
    const art = game.me5e.moduleArt.map.get(uuid);
  ...
  /** @inheritDoc */
  prepareData() {
    if ( this.system.modelProvider !== me5e ) return super.prepareData();
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      ...
      if ( effect.parent?.getFlag("me5e", "riders.effect")?.includes(effect.id) ) continue;
      yield effect;
    }
  }
  ...
  static async fetchExisting(uuid, options={}) {
    ...
    if ( !actor ) throw new Error(game.i18n.format("ME5E.ACTOR.Warning.NoActor", { uuid }));
    ...
    const localActor = game.actors.find(a => {
      ...
      return (a.getFlag("me5e", "isAutoImported") || a.getFlag("me5e", "summonedCopy")) // Back-compat
    ...
    if ( !game.user.can("ACTOR_CREATE") ) throw new Error("ME5E.ACTOR.Warning.CreateActor");
    ...
    if ( actor.pack ) {
      ...
      return game.actors.importFromCompendium(game.packs.get(actor.pack), actor.id, {
        "flags.me5e.isAutoImported": true
      });
    } else {
  ...
  async getPreferredArtwork() {
    if ( !this._preferredArtwork ) {
      const showTokenPortrait = this.getFlag("me5e", "showTokenPortrait") === true;
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("me5e", "summon.origin");
    if ( origin && this.token?.id ) {
      const { collection, primaryId } = foundry.utils.parseUuid(origin);
      me5e.registry.summons.track(collection?.get?.(primaryId)?.uuid, this.uuid);
    }
  }
  ...
  getConcentrationDC(damage) {
    return Math.clamp(
      Math.floor(damage / 2), 10, game.settings.get("me5e", "rulesVersion") === "modern" ? 30 : Infinity
    );
  }
  ...
  getCRExp(cr) {
    ...
    return CONFIG.ME5E.CR_EXP_LEVELS[cr] ?? Object.values(CONFIG.ME5E.CR_EXP_LEVELS).pop();
  }
   ...
   * @param {string} key      A key in `ME5E.conditionEffects`.
   * @returns {boolean}       Whether the actor is affected.
   */
  hasConditionEffect(key) {
    const props = CONFIG.ME5E.conditionEffects[key] ?? new Set();
    ...
    const applyExhaustion = (level !== null) && !imms.has("exhaustion")
      && (game.settings.get("me5e", "rulesVersion") === "legacy");
  ...
  _prepareSpellcasting() {
    ...
    const progression = Object.values(CONFIG.ME5E.spellcasting).reduce((acc, model) => {
    ...
    for ( const [type, model] of Object.entries(CONFIG.ME5E.spellcasting) ) {
  ...
  static computeClassProgression(progression, cls, {actor, spellcasting, count=1}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `me5e.computeLeveledProgression`).
     * @param {object} progression                    Spellcasting progression data. *Will be mutated.*
     ...
     * @function me5e.computeSpellcastingProgression
     * @memberof hookEvents
     */
    const allowed = Hooks.call(
      `me5e.compute${type.capitalize()}Progression`, progression, actor, cls, spellcasting, count
    );
    const model = CONFIG.ME5E.spellcasting[type];
    ...
    if ( model.isSingleLevel ) {
      if ( foundry.utils.getDefiningClass(this, "computePactProgression") !== Actor5e ) {
        foundry.utils.logCompatibilityWarning("Actor5e.computePactProgression is deprecated. Please use "
          + "SpellcastingModel#computeProgression instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
  ...
  static prepareSpellcastingSlots(spells, type, progression, {actor}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `me5e.prepareLeveledSlots`).
     * @param {object} spells        The `data.spells` object within actor's data. *Will be mutated.*
     ...
     * @function me5e.prepareSpellcastingSlots
     * @memberof hookEvents
     */
    const allowed = Hooks.call(`me5e.prepare${type.capitalize()}Slots`, spells, actor, progression);
    ...
    if ( model.isSingleLevel ) {
      if ( foundry.utils.getDefiningClass(this, "preparePactSlots") !== Actor5e ) {
        foundry.utils.logCompatibilityWarning("Actor5e.preparePactSlots is deprecated. Please use "
          + "SpellcastingModel#prepareSlots instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
  ...
  async _preCreate(data, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      const size = CONFIG.ME5E.actorSizes[this.system.traits.size || "med"].token ?? 1;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      ...
      if ( newSize && (newSize !== this.system.traits?.size) ) {
        let size = CONFIG.ME5E.actorSizes[newSize].token ?? 1;
    ...
    if ( "hp" in (this.system.attributes || {}) ) {
      ...
      foundry.utils.setProperty(options, "me5e.hp", { ...this.system.attributes.hp });
    }

    // Record previous exhaustion level.
    if ( Number.isFinite(foundry.utils.getProperty(changed, "system.attributes.exhaustion")) ) {
      foundry.utils.setProperty(options, "me5e.originalExhaustion", this.system.attributes.exhaustion);
    }
  }
  ...
  async applyDamage(damages, options={}) {
     ...
     * @function me5e.preApplyDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("me5e.preApplyDamage", this, amount, updates, options) === false ) return this;
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @function me5e.applyDamage
     * @memberof hookEvents
     */
    Hooks.callAll("me5e.applyDamage", this, amount, options);
  ...
  calculateDamage(damages, options={}) {
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function me5e.preCalculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("me5e.preCalculateDamage", this, damages, options) === false ) return false;
    ...
    const hasEffect = (category, type, properties) => {
      ...
      if ( !CONFIG.ME5E.damageTypes[type]?.isPhysical || !properties?.size ) return true;
      return !config.bypasses?.intersection(properties)?.size;
    };

    const skipped = type => {
      if ( options.only === "damage" ) return type in CONFIG.ME5E.healingTypes;
      if ( options.only === "healing" ) return type in CONFIG.ME5E.damageTypes;
      return false;
    };
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function me5e.calculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("me5e.calculateDamage", this, damages, options) === false ) return false;
  ...
  async beginConcentrating(activity, effectData={}) {
     ...
     * @function me5e.preBeginConcentrating
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("me5e.preBeginConcentrating", this, activity.item, effectData, activity) === false ) return;
     ...
     * @function me5e.createConcentrating
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor initiating concentration.
     ...
     * @param {Activity} activity         The activity that triggered the concentration.
     */
    Hooks.callAll("me5e.beginConcentrating", this, activity.item, effect, activity);
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("me5e", "item") ?? {};
     ...
     * @function me5e.preEndConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("me5e.preEndConcentration", this, effect) === false) return [];

    await effect.delete();

    /**
     * A hook that is called after a concentration effect is deleted.
     * @function me5e.endConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor ending concentration.
     * @param {ActiveEffect5e} effect     The ActiveEffect that was deleted.
     */
    Hooks.callAll("me5e.endConcentration", this, effect);
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    if ( ability in CONFIG.ME5E.abilities ) dataset.ability = ability;
    ...
    return ChatMessage.implementation.create({
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/me5e/templates/chat/roll-request-card.hbs",
        {
          buttons: [{
  ...
  _isRemarkableAthlete(ability) {
    return (game.settings.get("me5e", "rulesVersion") === "legacy") && this.getFlag("me5e", "remarkableAthlete")
  ...
  addRollExhaustion(parts, data) {
    if ( (game.settings.get("me5e", "rulesVersion") !== "modern") || !this.system.attributes?.exhaustion ) return;
    const amount = this.system.attributes.exhaustion * (CONFIG.ME5E.conditionTypes.exhaustion?.reduction?.rolls ?? 0);
    if ( amount ) {
  ...
  static async handleSkillCheckRequest(actor, request, config, { event }={}) {
    const data = {};
    foundry.utils.setProperty(data, "flags.me5e.requestResult", { actorUuid: actor.uuid, requestId: request.id });
  ...
  async rollSkill(config={}, dialog={}, message={}) {
    ...
    const skillLabel = CONFIG.ME5E.skills[config.skill]?.label ?? "";
    const ability = config.ability ?? this.system.skills[config.skill]?.ability ?? CONFIG.ME5E.skills[config.skill]?.ability ?? "";
  ...
  async rollToolCheck(config={}, dialog={}, message={}) {
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("ME5E.ToolPromptTitle", { tool: toolLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async #rollSkillTool(type, config={}, dialog={}, message={}) {
    ...
    const toolConfig = CONFIG.ME5E.tools[config.tool] ?? CONFIG.ME5E.vehicleTypes[config.tool];
    ...
    const hostActor = this.isPolymorphed && this.flags?.me5e?.transformOptions?.mergeSkills && (type === "skill")
      ? game.actors.get(this.flags.me5e?.originalActor) : null;
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      reliableTalent: (relevant?.value >= 1) && this.getFlag("me5e", "reliableTalent")
    }, config);
    ...
    const abilityLabel = CONFIG.ME5E.abilities[abilityId]?.label ?? "";
     ...
     * @param {object} data
     * @param {string} data.ability   Ability used as defined in `CONFIG.ME5E.abilities`.
     * @param {string} [data.skill]   ID of the skill that was rolled as defined in `CONFIG.ME5E.skills`.
     * @param {string} [data.tool]    ID of the tool that was rolled as defined in `CONFIG.ME5E.tools`.
    ...
    Hooks.callAll(`me5e.roll${name}`, rolls, data);
    Hooks.callAll(`me5e.roll${name}V2`, rolls, data);
  ...
  _buildSkillToolConfig(type, hostActor, process, config, formData, index) {
    ...
    const ability = this.system.abilities?.[abilityId];
    const { calculateSkillToolProficiency } = me5e.dataModels.actor.CommonTemplate;
  ...
  async rollAbilityCheck(config={}, dialog={}, message={}) {
    const abilityLabel = CONFIG.ME5E.abilities[config.ability]?.label ?? "";
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
  ...
  async rollSavingThrow(config={}, dialog={}, message={}) {
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("ME5E.SavePromptTitle", { ability: abilityLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async #rollD20Test(type, config={}, dialog={}, message={}) {
    ...
    const abilityConfig = CONFIG.ME5E.abilities[config.ability];
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        ...
        flavor: game.i18n.format(
          `ME5E.${type === "check" ? "Ability" : "Save"}PromptTitle`, { ability: abilityConfig?.label ?? "" }
        ),
        speaker: ChatMessage.getSpeaker({ actor: this })
      }
    }, message);
     ...
     * @function me5e.rollAbilityCheck
     * @function me5e.rollSavingThrow
     * @memberof hookEvents
     * @param {D20Roll[]} rolls       The resulting rolls.
     * @param {object} data
     * @param {string} data.ability   ID of the ability that was rolled as defined in `CONFIG.ME5E.abilities`.
     * @param {Actor5e} data.subject  Actor for which the roll has been performed.
     */
    Hooks.callAll(`me5e.roll${name}`, rolls, { ability: config.ability, subject: this });
  ...
  async rollDeathSave(config={}, dialog={}, message={}) {
    ...
    if ( (this.system.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3) ) {
      ui.notifications.warn("ME5E.DeathSaveUnnecessary", { localize: true });
      return null;
    }
    ...
    const messageConfig = foundry.utils.mergeObject({
      data: {
        ...
        flavor: game.i18n.localize("ME5E.DeathSavingThrow")
      }
    }, message);
     ...
     */
    if ( Hooks.call("me5e.rollDeathSave", rolls, details) === false ) return returnValue;
    if ( Hooks.call("me5e.rollDeathSaveV2", rolls, details) === false ) return returnValue;
     ...
     * @function me5e.postRollDeathSave
     * @memberof hookEvents
     * @param {D20Roll[]} rolls                  The resulting rolls.
     * @param {object} data
     ...
     * @param {Actor5e} data.subject             Actor for which the death saving throw has been rolled.
     */
    Hooks.callAll("me5e.postRollDeathSave", rolls, { message: resultsMessage, subject: this });
  ...
  async rollConcentration(config={}, dialog={}, message={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ability: (conc.ability in CONFIG.ME5E.abilities) ? conc.ability : CONFIG.ME5E.defaultAbilities.concentration,
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("ME5E.SavePromptTitle", { ability: game.i18n.localize("ME5E.Concentration") })
        }
      }
    }, dialog);
     ...
     * @function me5e.rollConcentration
     * @memberof hookEvents
     * @param {D20Roll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.actor  Actor for which the saving throw has been rolled.
     */
    Hooks.callAll("me5e.rollConcentration", rolls, { subject: this });
    Hooks.callAll("me5e.rollConcentrationV2", rolls, { subject: this });
  ...
  getInitiativeRollConfig(options={}) {
    ...
    const abilityId = init?.ability || CONFIG.ME5E.defaultAbilities.initiative;
    const ability = this.system.abilities?.[abilityId];
    ...
    let { parts, data } = CONFIG.Dice.D20Roll.constructParts({
      ...
      abilityCheckBonus: this.system.bonuses?.abilities?.check,
      alert: flags.initiativeAlert && (game.settings.get("me5e", "rulesVersion") === "legacy") ? 5 : null
    }, rollData);
     ...
     * @function me5e.preConfigureInitiative
     * @memberof hookEvents
     * @param {Actor5e} subject              The Actor that is rolling initiative.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     */
    Hooks.callAll("me5e.preConfigureInitiative", this, rollConfig);
  ...
  async rollInitiativeDialog(rollOptions={}) {
    ...
    if ( config.rolls[0].options?.fixed === undefined ) {
      const dialog = { options: { title: game.i18n.localize("ME5E.InitiativeRoll") } };
  ...
  async rollInitiative(options={}, rollOptions={}) {
     ...
     * @function me5e.preRollInitiative
     * @memberof hookEvents
     * @param {Actor5e} actor  The Actor that is rolling initiative.
     * @param {D20Roll} roll   The initiative roll.
     */
    if ( Hooks.call("me5e.preRollInitiative", this, this._cachedInitiativeRoll) === false ) {
  ...
  async rollHitDie(config={}, dialog={}, message={}) {
    ...
    if ( this.type === "npc" ) {
      ...
      if ( !this.system.attributes.hd.value ) {
        ui.notifications.error(game.i18n.format("ME5E.HitDiceNPCWarn", {name: this.name}));
        return null;
      }
    }
    ...
    // Otherwise check classes
      ...
      if ( !cls ) {
        ui.notifications.error(game.i18n.format("ME5E.HitDiceWarn", {name: this.name, formula: config.denomination}));
        return null;
      }
    }
    const rulesVersion = game.settings.get("me5e", "rulesVersion");
     ...
     * @returns {boolean}                  Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("me5e.rollHitDie", rolls, { subject: this, updates }) === false ) return returnValue;
    if ( Hooks.call("me5e.rollHitDieV2", rolls, { subject: this, updates }) === false ) return returnValue;
     ...
     * @function me5e.postRollHitDie
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.subject  Actor for which the roll was performed.
     */
    Hooks.callAll("me5e.postRollHitDie", rolls, { subject: this });
  ...
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("ME5E.ADVANCEMENT.HitPoints.Roll", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
     ...
     * @function me5e.rollClassHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("me5e.rollClassHitPoints", this, roll);
   ...
   * @param {boolean} [options.chatMessage=true]  Display the chat message for this roll.
   * @returns {Promise<Roll>}                     The completed roll.
   * @see {@link me5e.preRollNPCHitPoints}
   */
  async rollNPCHitPoints({ chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("ME5E.HPFormulaRollMessage");
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
     ...
     * @function me5e.preRollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor            Actor for which the hit points are being rolled.
     * @param {object} rollData
     * @param {string} rollData.formula  The string formula to parse.
     ...
     * @param {object} messageData       The data object to use when creating the message.
     */
    Hooks.callAll("me5e.preRollNPCHitPoints", this, rollData, messageData);
     ...
     * @function me5e.rollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("me5e.rollNPCHitPoints", this, roll);
  ...
  async shortRest(config={}) {
    ...
    if ( !game.user.isGM && !game.settings.get("me5e", "allowRests") && !config.request ) {
      ui.notifications.warn("ME5E.REST.Warning.OnlyByRequest", { localize: true, log: false });
      return;
    }

    const clone = this.clone();
    const restConfig = CONFIG.ME5E.restTypes.short;
    config = foundry.utils.mergeObject({
      type: "short", dialog: true, chat: true, newDay: false, advanceTime: false, autoHD: false, autoHDThreshold: 3,
      duration: CONFIG.ME5E.restTypes.short.duration[game.settings.get("me5e", "restVariant")],
  ...
  async longRest(config={}) {
    ...
    const clone = this.clone();
    const restConfig = CONFIG.ME5E.restTypes.long;
    config = foundry.utils.mergeObject({
      type: "long", dialog: true, chat: true, newDay: true, advanceTime: false,
      duration: restConfig.duration[game.settings.get("me5e", "restVariant")],
      recoverTemp: restConfig.recoverTemp, recoverTempMax: restConfig.recoverTempMax,
      exhaustionDelta: restConfig.exhaustionDelta
    }, config);
     ...
     * @function me5e.longRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being continued.
     */
    if ( Hooks.call("me5e.longRest", this, config) === false ) return;
  ...
  async _rest(config, result={}) {
     ...
     */
    if ( Hooks.call("me5e.preRestCompleted", this, result, config) === false ) return result;
     ...
     */
    Hooks.callAll("me5e.restCompleted", this, result, config);

    if ( config.advanceBastionTurn && game.user.isGM && game.settings.get("me5e", "bastionConfiguration").enabled
      && this.itemTypes.facility.length ) await me5e.bastion.advanceAllFacilities(this);
  ...
  async _displayRestResultMessage(config, result) {
    ...
    const typeConfig = CONFIG.ME5E.restTypes[config.type] ?? {};

    // Determine the chat message to display
    let message;
    if ( diceRestored && healthRestored ) message = `ME5E.REST.${length}.Result.Full`;
    else if ( longRest && !diceRestored && healthRestored ) message = "ME5E.REST.Long.Result.HitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "ME5E.REST.Long.Result.HitDice";
    else message = `ME5E.REST.${length}.Result.Short`;
    ...
    let chatData = {
      content: game.i18n.format(message, {
        name: this.name,
        dice: game.i18n.format(`ME5E.HITDICE.Counted.${pr.select(dhd)}`, { number: formatNumber(dhd) }),
        health: game.i18n.format(`ME5E.HITPOINTS.Counted.${pr.select(dhp)}`, { number: formatNumber(dhp) })
      }),
    ...
    if ( config.request ) foundry.utils.setProperty(chatData, "flags.me5e.requestResult", {
  ...
  createRestFlavor(config, result) {
    ...
    if ( result?.newDay ?? config.newDay ) parts.push(game.i18n.localize("ME5E.REST.NewDay.Label").toLowerCase());
  ...
  _getRestHitDiceRecovery({ maxHitDice, fraction, ...config }={}, result={}) {
    ...
    fraction ??= game.settings.get("me5e", "rulesVersion") === "modern" ? 1 : 0.5;
  ...
  _getRestHitPointRecovery({ recoverTemp, recoverTempMax, ...config }={}, result={}) {
    const restConfig = CONFIG.ME5E.restTypes[config.type ?? "long"];
    const hp = this.system.attributes?.hp;
  ...
  _prepareMovementAttribution() {
    ...
    const unit = CONFIG.ME5E.movementUnits[units]?.formattingUnit;
    const formatValue = value => `<span class="value">${
      unit ? formatLength(value ?? 0, unit, { parts: true })
        : `${value ?? 0} <span class="units">${units}</span>`
    }</span>`;
    return Object.entries(CONFIG.ME5E.movementTypes).reduce((html, [k, { label }]) => {
  ...
  async _prepareArmorClassAttribution({ title }={}) {
    ...
    const cfg = CONFIG.ME5E.armorClasses[ac.calc];
    const attribution = [];

    if ( ac.calc === "flat" ) {
      attribution.push({
        label: game.i18n.localize("ME5E.ArmorClassFlat"),
    ...
    switch ( ac.calc ) {
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("ME5E.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("ME5E.PropertyBase");
        if ( armorInFormula ) label = this.armor?.name ?? game.i18n.localize("ME5E.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.shield?.name ?? game.i18n.localize("ME5E.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
    ...
    if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("ME5E.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
  ...
  getOriginalStats() {
    ...
    if ( this.isPolymorphed ) {
      const transformOptions = this.flags.me5e?.transformOptions;
      const original = game.actors?.get(this.flags.me5e?.originalActor);
      if ( original ) {
  ...
  async transformInto(source, settings=new TransformationSetting(), options={}) {
    ...
    if ( !allowed && !game.user.isGM ) {
      ui.notifications.warn("ME5E.TRANSFORM.Warning.NoPermission", { localize: true });
      return null;
    }
    ...
    if ( settings.keep.has("self") ) {
      ...
      o.name = o.prototypeToken.name = `${o.name} (${game.i18n.localize("ME5E.TRANSFORM.Preset.Appearance.Label")})`;
    }
    ...
    if ( !settings.keep.has("self") ) {
      ...
      for ( let k of Object.keys(abilities) ) {
        ...
        const type = CONFIG.ME5E.abilities[k]?.type;
      ...
      const spellIdentifiers = settings.spellLists.size ? new Set(
        Array.from(settings.spellLists)
          .map(id => me5e.registry.spellLists.forType(id))
      ...
      if ( !settings.keep.has("class") && ("cr" in d.system.details) ) {
        if ( settings.keep.has("hp") ) {
          let profOverride = d.effects.findSplice(e => e._id === staticID("me5eTransformProf"));
          if ( !profOverride ) profOverride = new ActiveEffect.implementation({
            _id: staticID("me5eTransformProf"),
            name: game.i18n.localize("ME5E.Proficiency"),
          ...
          const cls = new me5e.dataModels.item.ClassData({ levels: d.system.details.cr });
          d.items.push({
            type: "class",
            name: game.i18n.localize("ME5E.TRANSFORM.TemporaryClass"),
            system: cls.toObject()
          });
        }
      }
      ...
      d.effects = d.effects.filter(e => {
        ...
        if ( settings.keep.has("hp") && !settings.keep.has("class") && (e._id === staticID("me5eTransformProf")) ) {
          return true;
        }
    ...
    const previousActorIds = this.getFlag("me5e", "previousActorIds") || [];
    previousActorIds.push(this._id);
    foundry.utils.setProperty(d.flags, "me5e.previousActorIds", previousActorIds);
    ...
    if ( this.isToken ) {
      ...
      if ( !this.token.flags.me5e?.previousActorData ) {
        ...
        foundry.utils.setProperty(tokenData, "flags.me5e.previousActorData", previousActorData);
        foundry.utils.setProperty(tokenData, "flags.me5e.previousTokenData", previousTokenData);
      }
     ...
     */
    Hooks.callAll("me5e.transformActor", this, source, d, settings, options);
    Hooks.callAll("me5e.transformActorV2", this, source, d, settings, options);
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.me5e.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.me5e.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.me5e.isPolymorphed", true);
      if ( !t.document.flags.me5e?.previousTokenData ) {
        const previousTokenData = { texture: {} };
        for ( const k of [...tokenPropsFromSource, ...tokenPropsFromSelf, "name"] ) {
        ...
        foundry.utils.setProperty(newTokenData, "flags.me5e.previousTokenData", previousTokenData);
      }
      return newTokenData;
    });
  ...
  async revertOriginalForm(options={}) {
    ...
    if ( !this.isOwner ) {
      ui.notifications.warn("ME5E.TRANSFORM.Warning.NoOwnership", { localize: true });
      return null;
    }
     ...
     * @function me5e.revertOriginalForm
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("me5e.revertOriginalForm", this, options);
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("ME5E.TRANSFORM.Warning.OriginalActor", {
          reference: this.getFlag("me5e", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("me5e", "previousActorData");
      ...
      foundry.utils.mergeObject(tokenUpdate, this.token.getFlag("me5e", "previousTokenData"));
      tokenUpdate.sight = prototypeTokenData.sight;
      tokenUpdate.detectionModes = prototypeTokenData.detectionModes;
      delete tokenUpdate.flags.me5e.previousActorData;
      delete tokenUpdate.flags.me5e.previousTokenData;
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        foundry.utils.mergeObject(update, t.document.getFlag("me5e", "previousTokenData"));
        delete update.x;
        delete update.y;
        return update;
      });
    ...
    if ( game.user.isGM ) {
      ...
      const actorUpdates = polymorphedActorIds.filter(id => game.actors.get(id).isOwner).map(p => {
        return { _id: p, "flags.me5e": { "-=isPolymorphed": null, "-=previousActorIds": null } };
      });
    ...
    if ( !foundry.utils.isEmpty(update) ) await original.update(update, { me5e: { concentrationCheck: false } });
    return original;
  }
  ...
  static addDirectoryContextOptions(app, entryOptions) {
    ...
    entryOptions.push({
      name: "ME5E.TRANSFORM.Action.Restore",
      ...
      name: "ME5E.Group.Primary.Set",
      icon: '<i class="fa-solid fa-star"></i>',
      callback: li => {
        game.settings.set("me5e", "primaryParty", { actor: game.actors.get(li.dataset.documentId ?? li.dataset.entryId) });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
      ...
      name: "ME5E.Group.Primary.Remove",
      icon: '<i class="fa-regular fa-star"></i>',
      callback: li => {
        game.settings.set("me5e", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
  ...
  static formatCreatureType(typeData) {
    ...
    } else if ( typeData.value in CONFIG.ME5E.creatureTypes ) {
      const code = CONFIG.ME5E.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? code.plural : code.label);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("ME5E.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.ME5E.actorSizes[typeData.swarm].label),
        type: localizedType
      });
    }
  ...
  async _onUpdate(data, options, userId) {
    ...
    if ( isHpUpdate && hp && !options.isRest && !options.isAdvancement ) {
      ...
      if ( Number.isInteger(changes.total) && (changes.total !== 0) ) {
         ...
         */
        Hooks.callAll(`me5e.${changes.total > 0 ? "heal" : "damage"}Actor`, this, changes, data, userId);
      }
    }
  }
  ...
  _onDelete(options, userId) {
    ...
    for ( const id in this.apps ) {
      ...
      if ( app instanceof me5e.applications.actor.GroupActorSheet ) delete this.apps[id];
    }
    ...
    if ( origin ) {
      const { collection, primaryId } = foundry.utils.parseUuid(origin);
      me5e.registry.summons.untrack(collection?.get?.(primaryId)?.uuid, this.uuid);
    }
  }
  ...
  _displayTokenEffect(changes) {
    ...
    const fill = CONFIG.ME5E.tokenHPColors[key];
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      const originalExhaustion = foundry.utils.getProperty(options, "me5e.originalExhaustion");
      return effect.update({ "flags.me5e.exhaustionLevel": level }, { me5e: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.me5e.exhaustionLevel": level });
  ...
  updateBloodied(options) {
    const hp = this.system.attributes?.hp;
    if ( !hp?.effectiveMax || (game.settings.get("me5e", "bloodied") === "none") ) return;

    const effect = this.effects.get(ActiveEffect5e.ID.BLOODIED);
    if ( hp.value > hp.effectiveMax * CONFIG.ME5E.bloodied.threshold ) return effect?.delete();
    if ( effect ) return;

    return ActiveEffect.implementation.create({
      _id: ActiveEffect5e.ID.BLOODIED,
      name: game.i18n.localize(CONFIG.ME5E.bloodied.name),
      img: CONFIG.ME5E.bloodied.img,
      statuses: ["bloodied"]
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("me5e", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("me5e", "encumbrance") === "variant";
    ...
    const effectData = { ...CONFIG.ME5E.encumbrance.effects[statuses[0]], statuses };
    if ( effect ) {
      const originalEncumbrance = effect.statuses.first();
      return effect.update(effectData, { me5e: { originalEncumbrance } });
    }
  ...
  static getDefaultArtwork(actorData={}) {
    const img = CONFIG.ME5E.defaultArtwork.Actor[actorData.type];
  ...
  static computeLeveledProgression(progression, actor, cls, spellcasting, count) {
    foundry.utils.logCompatibilityWarning("Actor5e.computeLeveledProgression is deprecated. Please use "
      + "SpellcastingModel#computeProgression instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
    CONFIG.ME5E.spellcasting.spell.computeProgression(progression, actor, cls, spellcasting, count);
  }
  ...
  static computePactProgression(progression, actor, cls, spellcasting, count) {
    foundry.utils.logCompatibilityWarning("Actor5e.computePactProgression is deprecated. Please use "
      + "SpellcastingModel#computeProgression instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
    CONFIG.ME5E.spellcasting.pact.computeProgression(progression, actor, cls, spellcasting, count);
  }
  ...
  static prepareAltSlots(spells, actor, progression, key, table) {
    foundry.utils.logCompatibilityWarning("Actor.prepareAltSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
    const model = CONFIG.ME5E.spellcasting[key];
    if ( !model ) return;
  ...
  static prepareLeveledSlots(spells, actor, progression) {
    foundry.utils.logCompatibilityWarning("Actor.prepareLeveledSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
    CONFIG.ME5E.spellcasting.spell.prepareSlots(spells, actor, progression);
  }
  ...
  static preparePactSlots(spells, actor, progression) {
    foundry.utils.logCompatibilityWarning("Actor.preparePactSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "ME5e 5.1", until: "ME5e 5.4" });
    CONFIG.ME5E.spellcasting.pact.prepareSlots(spells, actor, progression);
  }
}
