 ...
 * @param {string} trait   Trait as defined in `CONFIG.ME5E.traits`.
 * @returns {object|void}
 */
...
export function actorKeyPath(trait) {
  const traitConfig = CONFIG.ME5E.traits[trait];
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.ME5E.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.ME5E.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.ME5E[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.ME5E[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.ME5E[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.ME5E[key] ?? {});
      return obj;
    }, {});
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.ME5E.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`ME5E.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.ME5E.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.ME5E[traitConfig.configKey ?? trait] ?? {};
  ...
  // Wildcards (e.g. "Artisan's Tools", "any Artisan's Tools", "any 2 Artisan's Tools", or "2 other Artisan's Tools")
    ...
    const localization = `ME5E.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.ME5E[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      let baseItemId = CONFIG.ME5E[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("ME5E.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("ME5E.TraitConfigChooseWrapper", {
