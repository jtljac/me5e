...
export default class SizeAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/me5e/icons/svg/size.svg",
      title: game.i18n.localize("ME5E.ADVANCEMENT.Size.Title"),
      hint: game.i18n.localize("ME5E.ADVANCEMENT.Size.Hint"),
      apps: {
        config: SizeConfig,
        flow: SizeFlow
      }
  ...
  get automaticHint() {
    ...
    if ( this.configuration.sizes.size === 1 ) return game.i18n.format("ME5E.ADVANCEMENT.Size.DefaultHint.Single", {
      size: CONFIG.ME5E.actorSizes[this.configuration.sizes.first()].label
    });

    const listFormatter = new Intl.ListFormat(game.i18n.lang, { type: "disjunction" });
    return game.i18n.format("ME5E.ADVANCEMENT.Size.DefaultHint.Multiple", {
      sizes: listFormatter.format(this.configuration.sizes.map(s => CONFIG.ME5E.actorSizes[s].label))
    });
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return sizes.map(s => `<span class="tag">${CONFIG.ME5E.actorSizes[s].label}</span>`).join("");
  }
