...
export default class NPCActorSheet extends BaseActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/me5e/templates/actors/npc-header.hbs"
    },
    sidebarCollapser: {
      container: { classes: ["main-content"], id: "main" },
      template: "systems/me5e/templates/actors/parts/sidebar-collapser.hbs"
    },
    sidebar: {
      container: { classes: ["main-content"], id: "main" },
      template: "systems/me5e/templates/actors/npc-sidebar.hbs"
    },
    features: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/me5e/templates/actors/tabs/actor-features.hbs",
    ...
    inventory: {
      ...
      templates: [
        ...
        "systems/me5e/templates/inventory/encumbrance.hbs"
      ],
      scrollable: [""]
    },
    spells: {
    ...
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/me5e/templates/actors/tabs/npc-biography.hbs",
      scrollable: [""]
    },
    specialTraits: {
      classes: ["flexcol"],
  ...
  async _prepareFeaturesContext(context, options) {
    const sections = Object.entries(CONFIG.ME5E.activityActivationTypes).reduce((obj, [id, config], i) => {
    ...
    sections.passive = {
      id: "passive", label: "ME5E.Features", order: 0, items: [], minWidth: 210,
      columns: ["recovery", "uses", "roll", "formula", "controls"]
    };
    ...
    context.listControls = {
      ...
      list: "features",
      filters: [
        { key: "action", label: "ME5E.ACTIVATION.Type.Action.Label" },
        { key: "bonus", label: "ME5E.ACTIVATION.Type.BonusAction.Label" },
        { key: "reaction", label: "ME5E.ACTIVATION.Type.Reaction.Label" },
        { key: "legendary", label: "ME5E.ACTIVATION.Type.Legendary.Label" },
        { key: "lair", label: "ME5E.ACTIVATION.Type.Lair.Label" }
      ],
      sorting: [
  ...
  async _prepareHeaderContext(context, options) {
    ...
    for ( const res of ["legact", "legres"] ) {
      ...
      context[res] = Array.fromRange(max, 1).map(n => {
        ...
        return {
          n: max - n, filled,
          tooltip: `ME5E.${i18n}.Label`,
          label: game.i18n.format(`ME5E.${i18n}.Ordinal.${plurals.select(n)}`, { n }),
    ...
    if ( this._mode === this.constructor.MODES.PLAY ) {
      ...
      context.showInitiativeScore = game.settings.get("me5e", "rulesVersion") === "modern";
    }
    context.showLoyalty = context.important && game.settings.get("me5e", "loyaltyScore") && game.user.isGM;
  ...
  async _prepareSidebarContext(context, options) {
    ...
    if ( details.habitat.value.length || details.habitat.custom ) {
      ...
      context.habitat = [
        ...habitat.value.map(({ type, subtype }) => {
          let { label } = CONFIG.ME5E.habitats[type] ?? {};
          if ( label && (!any || (type === "any")) ) {
            if ( subtype ) label = game.i18n.format("ME5E.Habitat.Subtype", { type: label, subtype });
            return { label };
          }
          return null;
        }, []).filter(_ => _),
    ...
    if ( this.actor.system.skills.prc ) context.senses.push({
      ...
      label: game.i18n.localize("ME5E.PassivePerception"),
      value: this.actor.system.skills.prc.passive
    });

    // Skills & Tools
    const skillSetting = game.settings.get("me5e", "defaultSkills");
    ...
    // Speed
    context.speed = [
      ...Object.entries(CONFIG.ME5E.movementTypes).map(([k, { label }]) => {
        const value = attributes.movement[k];
        if ( !value ) return null;
        const data = { label, value };
        if ( (k === "fly") && attributes.movement.hover ) data.icons = [{
          icon: "fas fa-cloud", label: game.i18n.localize("ME5E.MOVEMENT.Hover")
        }];
        return data;
      }),
    ...
    if ( details?.treasure?.value.size ) {
      ...
      context.treasure = Array.from(details.treasure.value)
        .map(id => {
          const { label } = CONFIG.ME5E.treasure[id] ?? {};
  ...
  async _prepareSpecialTraitsContext(context, options) {
    ...
    context.flags.sections.unshift({
      label: game.i18n.localize("ME5E.NPC.Label"),
      fields: [{
        ...
        label: "ME5E.NPC.FIELDS.attributes.price.label",
        hint: "ME5E.NPC.FIELDS.attributes.price.hint",
        fields: [{
          ...
          choices: CONFIG.ME5E.currencies,
  ...
  async _prepareSpellsContext(context, options) {
    ...
    context.spellcasting.push({
      label: game.i18n.format("ME5E.SpellcastingClass", {
        class: spellcaster?.name ?? game.i18n.format("ME5E.NPC.Label")
      }),
      level: spellcaster?.system.levels ?? attributes.spell.level,
      ability: {
        ability, mod,
        label: CONFIG.ME5E.abilities[ability]?.label
      },
  ...
  async _prepareItem(item, ctx) {
    ...
    const isPassive = item.system.properties?.has("trait")
      || CONFIG.ME5E.activityActivationTypes[item.system.activities?.contents[0]?.activation.type]?.passive;
  ...
  async _onRender(context, options) {
    ...
    elements.innerText = xp === null ? "" : game.i18n.format("ME5E.ExperiencePoints.Format", {
