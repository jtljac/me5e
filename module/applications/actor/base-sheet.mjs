...
export default class ActorSheet5e extends ActorSheet {
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/me5e/templates/actors/limited-sheet.hbs";
    return `systems/me5e/templates/actors/${this.actor.type}-sheet.hbs`;
  }
  ...
  async getData(options) {
    ...
    for ( const [a, abl] of Object.entries(context.abilities) ) {
      ...
      abl.hover = CONFIG.ME5E.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.ME5E.abilities[a]?.label;
    ...
    ["skills", "tools"].forEach(prop => {
      for ( const [key, entry] of Object.entries(context[prop]) ) {
        entry.abbreviation = CONFIG.ME5E.abilities[entry.ability]?.abbreviation;
        entry.icon = this._getProficiencyIcon(entry.value);
        entry.hover = CONFIG.ME5E.proficiencyLevels[entry.value];
        entry.label = prop === "skills" ? CONFIG.ME5E.skills[key]?.label : Trait.keyLabel("tool", key);
  ...
  _getLabels() {
    ...
    // Currency Labels
    labels.currencies = Object.entries(CONFIG.ME5E.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});

    // Proficiency
    labels.proficiency = game.settings.get("me5e", "proficiencyModifier") === "dice"
  ...
  _getMovementSpeed(systemData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("ME5E.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("ME5E.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("ME5E.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("ME5E.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("ME5E.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("ME5E.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(systemData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.ME5E.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
  ...
  _prepareArmorClassAttribution(rollData) {
    ...
    switch ( ac.calc ) {
      ...
      default:
        ...
        if ( armorInFormula ) label = this.actor.armor?.name ?? game.i18n.localize("ME5E.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.actor.shield?.name ?? game.i18n.localize("ME5E.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
  ...
  _prepareTraits(systemData) {
    const traits = {};
    for ( const [trait, traitConfig] of Object.entries(CONFIG.ME5E.traits) ) {
      ...
      const choices = CONFIG.ME5E[traitConfig.configKey];
      if ( !data ) continue;
      ...
      if ( data.bypasses?.size ) {
        values = values.filter(t => {
          if ( !CONFIG.ME5E.physicalDamageTypes[t] ) return true;
          physical.push(t);
          return false;
        });
      }
      ...
      if ( physical.length ) {
        ...
        data.selected.physical = game.i18n.format("ME5E.DamagePhysicalBypasses", {
          damageTypes: damageTypesFormatter.format(physical.map(t => choices[t])),
          bypassTypes: bypassFormatter.format(data.bypasses.map(t => CONFIG.ME5E.physicalWeaponProperties[t]))
        });
      }
  ...
  _prepareSpellbook(context, spells) {
    ...
    const maxLevel = Array.fromRange(Object.keys(CONFIG.ME5E.spellLevels).length - 1, 1).reduce((max, i) => {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.ME5E.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.ME5E.spellLevels[lvl], levels[sl]);
      }
    }

    // Pact magic users have cantrips and a pact magic section
    if ( levels.pact && levels.pact.max ) {
      if ( !spellbook["0"] ) registerSection("spell0", 0, CONFIG.ME5E.spellLevels[0]);
      const l = levels.pact;
      const config = CONFIG.ME5E.spellPreparationModes.pact;
      const level = game.i18n.localize(`ME5E.SpellLevel${levels.pact.level}`);
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        ...
        if ( !spellbook[s] ) {
          const l = levels[mode] || {};
          const config = CONFIG.ME5E.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.ME5E.spellLevels[s], {levels: levels[sl]});
      }
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.ME5E.proficiencyLevels`.
  ...
  _disableOverriddenFields(html) {
    ...
    for ( const override of Object.keys(foundry.utils.flattenObject(this.actor.overrides)) ) {
      html.find(`input[name="${override}"],select[name="${override}"]`).each((i, el) => {
        el.disabled = true;
        el.dataset.tooltip = "ME5E.ActiveEffectOverrideWarning";
      });
      ...
      for ( const [key, regex] of Object.entries(proficiencyToggles) ) {
        ...
        if ( match ) {
          ...
          toggle.attr("data-tooltip", "ME5E.ActiveEffectOverrideWarning");
        }
      }

      const [, spell] = override.match(/system\.spells\.(spell\d)\.override/) || [];
      if ( spell ) {
        html.find(`.spell-max[data-level="${spell}"]`).attr("data-tooltip", "ME5E.ActiveEffectOverrideWarning");
      }
    }
  }
  ...
  _onItemContext(element) {
    ...
    // Items
      ...
      Hooks.call("me5e.getItemContextOptions", item, ui.context.menuItems);
    }
  }
  ...
  _getItemContextOptions(item) {
    ...
    if ( ("attunement" in item.system) && (item.system.attunement !== CONFIG.ME5E.attunementTypes.NONE) ) {
      const isAttuned = item.system.attunement === CONFIG.ME5E.attunementTypes.ATTUNED;
      options.push({
        name: isAttuned ? "ME5E.ContextMenuActionUnattune" : "ME5E.ContextMenuActionAttune",
        icon: "<i class='fas fa-sun fa-fw'></i>",
        callback: () => item.update({
          "system.attunement": CONFIG.ME5E.attunementTypes[isAttuned ? "REQUIRED" : "ATTUNED"]
        })
      });
    }
    ...
    if ( "equipped" in item.system ) options.push({
      name: item.system.equipped ? "ME5E.ContextMenuActionUnequip" : "ME5E.ContextMenuActionEquip",
    ...
    if ( ("preparation" in item.system) && (item.system.preparation?.mode === "prepared") ) options.push({
      name: item.system?.preparation?.prepared ? "ME5E.ContextMenuActionUnprepare" : "ME5E.ContextMenuActionPrepare",
      icon: "<i class='fas fa-sun fa-fw'></i>",
  ...
  async _onDropSingleItem(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      ui.notifications.warn(game.i18n.format("ME5E.ActorWarningInvalidItem", {
  ...
  _onDropResetData(itemData) {
    ...
    if ( "attunement" in itemData.system ) {
      itemData.system.attunement = Math.min(itemData.system.attunement, CONFIG.ME5E.attunementTypes.REQUIRED);
    }
  }
  ...
  async _onItemSummary(event) {
    ...
    if ( li.hasClass("expanded") ) {
      ...
      const summary = $(await renderTemplate("systems/me5e/templates/items/parts/item-summary.hbs", chatData));
  ...
  _onItemCreate(event) {
    ...
    if ( type === "class" && (this.actor.system.details.level + 1 > CONFIG.ME5E.maxLevel) ) {
    ...
    const itemData = {
      name: game.i18n.format("ME5E.ItemNew", {type: game.i18n.localize(CONFIG.Item.typeLabels[type])}),
      type: type,
