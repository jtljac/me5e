...
export default class AbilityUseDialog extends Dialog {
  ...
  static async create(item) {
    ...
    const data = {
      item: item,
      title: game.i18n.format("ME5E.AbilityUseHint", {type: game.i18n.localize(`ME5E.ItemType${item.type.capitalize()}`), name: item.name}),
    ...
    const label = game.i18n.localize(`ME5E.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.ME5E.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    if ( !canCast ) data.errors.push(game.i18n.format("ME5E.SpellCastNoSlots", {
      level: CONFIG.ME5E.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "ME5E.AbilityUseChargedHint" : "ME5E.AbilityUseRechargeHint", {
        type: game.i18n.localize(`ME5E.ItemType${item.type.capitalize()}`)
      });
    }
    ...
    if ( item.type === "consumable" ) {
      ...
      else if ( item.system.quantity === 1 && uses.autoDestroy ) str = "ME5E.AbilityUseConsumableDestroyHint";
      else if ( item.system.quantity > 1 ) str = "ME5E.AbilityUseConsumableQuantityHint";
