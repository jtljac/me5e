...
export default class TransformSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/me5e/templates/activity/transform-effect.hbs",
      templates: [
        ...super.PARTS.effect.templates,
        "systems/me5e/templates/activity/parts/transform-profiles.hbs",
        "systems/me5e/templates/activity/parts/transform-settings.hbs"
      ]
    }
  };
  ...
  async _prepareEffectContext(context, options) {
    ...
    const settings = new TransformationSetting({
      ...(context.source.transform.customize ? context.source.settings
        : CONFIG.ME5E.transformation.presets[context.source.transform.preset]?.settings ?? {}),
    ...
    context.presetOptions = [
      { value: "", label: game.i18n.localize("ME5E.TRANSFORM.Preset.Default") },
      { rule: true },
      ...Object.entries(CONFIG.ME5E.transformation.presets)
        .map(([value, { label }]) => ({ value, label }))
    ];

    context.creatureSizeOptions = Object.entries(CONFIG.ME5E.actorSizes)
      .map(([value, { label }]) => ({ value, label }));
    context.creatureTypeOptions = Object.entries(CONFIG.ME5E.creatureTypes)
      .map(([value, { label }]) => ({ value, label }));
    context.movementTypeOptions = Object.entries(CONFIG.ME5E.movementTypes)
      .map(([value, { label }]) => ({ value, label }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("ME5E.TRANSFORM.FIELDS.transform.mode.Direct") },
      { value: "cr", label: game.i18n.localize("ME5E.TRANSFORM.FIELDS.transform.mode.CR") }
    ];
  ...
  async _processSubmitData(event, submitData) {
    ...
    if ( submitData.transform?.customize && !this.activity._source.settings ) {
      ...
      submitData.settings = new TransformationSetting(foundry.utils.mergeObject({
        ...(CONFIG.ME5E.transformation.presets[preset]?.settings ?? {}),
