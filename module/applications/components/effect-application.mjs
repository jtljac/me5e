...
export default class EffectApplicationElement extends TargetedApplicationMixin(ChatTrayElement) {
    ...
    if ( !this.effectsList || !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("ME5E.Effects")}</span>
    ...
    for ( const effect of this.effects ) {
      ...
      li.innerHTML = `
        ...
        <button class="apply-effect" type="button" data-action="applyEffect"
                data-tooltip aria-label="${game.i18n.localize("ME5E.EffectsApplyTokens")}">
  ...
  buildTargetListEntry({ uuid, name }) {
    ...
    li.innerHTML = `
      ...
      <div class="checkbox">
        <me5e-checkbox name="${uuid}"${checked}${disabled}></me5e-checkbox>
      </div>
    `;
  ...
  async _applyEffectToActor(effect, actor) {
    const concentration = this.chatMessage.getAssociatedActor()?.effects
      .get(this.chatMessage.getFlag("me5e", "use.concentrationId"));
    const origin = concentration ?? effect;
    if ( !game.user.isGM && !actor.isOwner ) {
      throw new Error(game.i18n.localize("ME5E.EffectApplyWarningOwnership"));
    }
    ...
    const effectFlags = {
      flags: {
        me5e: {
          ...
          scaling: this.chatMessage.getFlag("me5e", "scaling"),
          spellLevel: this.chatMessage.getFlag("me5e", "use.spellLevel")
        }
      }
    };
    ...
    if ( !game.user.isGM && concentration && !concentration.isOwner ) {
      throw new Error(game.i18n.localize("ME5E.EffectApplyWarningConcentration"));
    }
  ...
  async _onApplyEffect(event) {
    ...
    for ( const target of this.targetList.querySelectorAll("[data-target-uuid]") ) {
      ...
      if ( !actor || !target.querySelector("me5e-checkbox")?.checked ) continue;
      try {
