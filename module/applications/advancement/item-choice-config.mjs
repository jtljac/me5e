...
export default class ItemChoiceConfig extends AdvancementConfig {
  ...
  static PARTS = {
    ...
    details: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/me5e/templates/advancement/item-choice-config-details.hbs"
    },
    spellConfig: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/me5e/templates/advancement/advancement-spell-config-section.hbs"
    },
    items: {
      container: { classes: ["column-container"], id: "column-center" },
      template: "systems/me5e/templates/advancement/item-choice-config-items.hbs"
    },
    levels: {
      container: { classes: ["column-container"], id: "column-right" },
      template: "systems/me5e/templates/advancement/item-choice-config-levels.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.levelRestrictionOptions = [
      ...
      {
        value: "available",
        label: game.i18n.localize("ME5E.ADVANCEMENT.ItemChoice.FIELDS.restriction.level.Available")
      },
      { rule: true },
      ...Object.entries(CONFIG.ME5E.spellLevels).map(([value, label]) => ({ value, label }))
    ];
    context.listRestrictionOptions = me5e.registry.spellLists.options;
    ...
    const model = CONFIG.ME5E.spellcasting[spell?.method];
    ...
    if ( spell?.method && !(spell.method in CONFIG.ME5E.spellcasting) ) {
      context.spellcastingMethods.push({ label: spell.method, value: spell.method });
    }

    context.typeOptions = [
      { value: "", label: game.i18n.localize("ME5E.ADVANCEMENT.ItemChoice.FIELDS.type.Any") },
      { rule: true },
    ...
    if ( this.advancement.configuration.type === "feat" ) {
      ...
      context.typeRestriction = {
        ...
        typeOptions: [
          { value: "", label: "" },
          ...Object.entries(CONFIG.ME5E.featureTypes).map(([value, { label }]) => ({ value, label }))
        ],
