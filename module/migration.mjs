...
function cleanActorData(actorData) {
  ...
  if ( actorData.flags.me5e ) {
    actorData.flags.me5e = foundry.utils.filterObject(actorData.flags.me5e, allowedFlags);
  }
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["system.attunement"] = CONFIG.ME5E.attunementTypes.NONE;
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.ME5E.itemRarity).find(key =>
    (CONFIG.ME5E.itemRarity[key].toLowerCase() === item.system.rarity.toLowerCase()) || (key === item.system.rarity)
  );
